...
const ChecklistCarousel: React.FC<ChecklistCarouselProps> = ({
  data,
  icon,
  title,
}) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-12 py-5">
      {title && (
        <h2 className="font-primary text-xl md:text-3xl font-bold text-black  text-center mb-3 md:mb-6">
          {title}
        </h2>
      )}
              ...
              <CarouselItem key={slideIndex}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6  h-full">
                  {slideCards.map((card, cardIndex) => (
                    <div
                      key={cardIndex}
                      className="bg-white rounded-xl p-4 md:p-6 border shadow-sm border-gray-200 h-full"
                    >
                      ...
                      <div className="space-y-2 md:space-y-3">
                        {card.description.map((item, itemIndex) => (
                          ...
                          >
                            ...
                            <span className="text-gray-600 font-secondary text-sm flex-1">
                              {item}
                            </span>
                          </div>
                        ))}
                  ...
                  {slideCards.length === 1 && !isMobile && <div></div>}
                </div>
              </CarouselItem>
            ))}
          </CarouselContent>
          ...
          {slidesData.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all cursor-pointer shadow-sm duration-200 ${
                current - 1 === index ? "bg-blue-600 " : "bg-gray-400"
              }`}
